...
case 'addprem':
    ...
    try {
        ...
        if (quoted) {
            if (isImage || quoted.mtype === 'imageMessage') {
                content = { image: await EliteProTech.downloadMediaMessage(quoted) };
            } else if (isVideo || quoted.mtype === 'videoMessage') {
                content = { video: await EliteProTech.downloadMediaMessage(quoted) };
            } else if (isAudio || quoted.mtype === 'audioMessage') {
                content = { audio: await EliteProTech.downloadMediaMessage(quoted) };
            } else if (isText || quoted.mtype === 'conversation' || quoted.mtype === 'extendedTextMessage') {
                content = { text: quoted.text || quoted.conversation };
            }
        } 
        // Check for tagged media
        else if (isMedia) {
            if (isImage) {
    ...
    try {
        if (m.isGroup) {
            ...
            const ppUrl = await EliteProTech.profilePictureUrl(mentioned, 'image')
            
            if (!ppUrl) return reply('No profile picture found for this user')
            
            await EliteProTech.sendMessage(m.chat, { 
                image: { url: ppUrl },
                caption: `Profile picture of @${mentioned.split('@')[0]}`,
                mentions: [mentioned]
            }, { quoted: m })
        } else {
            // Private chat handling
            const user = m.sender
            const ppUrl = await EliteProTech.profilePictureUrl(user, 'image')
            
            if (!ppUrl) return reply('You dont have a profile picture!')
...
case 'getgcpp': {
    try {
        ...
        const gcJid = m.chat
        const ppUrl = await EliteProTech.profilePictureUrl(gcJid, 'image')
        
        if (!ppUrl) return reply('This group has no profile picture!')
        
        await EliteProTech.sendMessage(m.chat, { 
            image: { url: ppUrl },
            caption: `Profile picture of ${groupName || 'this group'}`
        }, { quoted: m })
        
    } catch (error) {
        console.error('Error in getgcpp:', error)
